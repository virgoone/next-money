...
export function formatOriginalPrice(
  ...
  return price ? currency + price.toFixed(fixedNumber || 2) : `${currency}0.00`;
}
...
const RatioAspect = {
  [Ratio.r1]: "aspect-square",
  [Ratio.r2]: "aspect-[16/9]",
...
export function downloadImage(url: string) {
  fetch(url)
    ...
    .then((blob) => {
      ...
      link.href = url;
      link.download = `fluxaipro.art.${getMime(url)}`;
